...
◊(define-meta title "Wolf, Goat and Cabbage puzzle, in Coq")
◊(define-meta date "2021-09-09")
◊(define-meta tags ("coq" "puzzles" "formal methods"))

◊link[#:rel "stylesheet" #:href "/css/alectryon.css" #:type "text/css"]{}
◊link[#:rel "stylesheet" #:href "/css/tango_subtle.min.css" #:type "text/css"]{}

◊ul{
◊li{◊hyperlink["#introduction"]{Introduction}}
◊li{◊hyperlink["#basic-definitions"]{Basic definitions}}
◊li{◊hyperlink["#first-attempt-problematic"]{First attempt (problematic!)}}
◊li{◊hyperlink["#second-attempt"]{Second attempt}}
◊li{◊hyperlink["#conclusions-and-issues"]{Conclusions and Issues}}
}


◊h1[#:id "introduction"]{Introduction}

◊p{Earlier this year, I attended the ◊hyperlink["https://fmie2021.github.io/"]{Formal Methods for the Informal
Engineer} workshop, and found that quite
...
◊hyperlink["https://coq.inria.fr/"]{interactive theorem prover Coq}, during the
◊hyperlink["https://youtu.be/5e7UdWzITyQ"]{nice tutorial} by Cody Roux.}

◊p{Some time later, after working thought the ◊hyperlink["https://softwarefoundations.cis.upenn.edu/lf-current/index.html"]{Logical
Foundations}
book from the Software Foundations series, I wanted to try and
practice it on my own on some toy problem. The ◊hyperlink["https://en.wikipedia.org/wiki/Wolf,_goat_and_cabbage_problem"]{"wolf, goat and
cabbage
problem"}
came to mind, and I have previously seems examples that show that the
◊hyperlink["https://alloytools.org/tutorials/online/frame-RC-1.html"]{Alloy
Analyzer} and
◊hyperlink["https://sudonull.com/post/29525-Formal-verification-on-the-example-of-the-wolf-goat-and-cabbage-problem"]{Z3}
...
◊p{In my first attempt, I managed to model and reproduce a known solution
...
◊p{The problem was that I was not constraining the contents of both river
...
to create my own version of the ◊code{auto} or ◊code{eauto} tactics, like ◊hyperlink["http://adam.chlipala.net/cpdt/"]{Adam
Chlipala’s ◊code{crush}}, but I don’t know
how to do that quite yet.}

◊p{Even with this second solution, there some issues with it and I’m
...
◊p{I should also say that I used
◊hyperlink["https://github.com/cpitclaudel/alectryon/"]{Alectryon} by Clément
...
◊hyperlink["https://www.philipzucker.com/translating-z3-to-coq/"]{post} about
...
◊h1[#:id "basic-definitions"]{Basic definitions}

◊p{The definition below did not change between attempts, and were shared
by both solutions.}

◊p{First, we need to import some modules so we can use a notation for
list literals. This is an amazing feature of Coq: the ability to
define custom notation.}

◊alectryon{From Coq Require Import Lists.List. Import ListNotations.}

◊p{Then, we define the "objects" that the farmer wants to take from one
...
◊alectryon{Inductive Object :=
| Wolf
| Goat
| Cabbage.}

◊p{In each step, the farmer must cross from one side of the river to the
...
◊alectryon{Inductive Direction :=
| Right
| Left.}

◊p{But some pairs of objects cannot be left unattended, else they eat
each other. Those pairs are "forbidden" when crossing the river. The
◊code{eats} function tells us if either of two objects eats the other, and
the ◊code{NoEats} predicate uses that to assert that list of objects,
representing one side of the river, contains no forbidden pairs of
objects.}

◊alectryon{Definition eats (o1 o2 : Object) :  bool :=
  match (o1, o2) with
  | (Wolf, Goat) => true
  | (Goat, Wolf) => true
  | (Goat, Cabbage) => true
...
◊h1[#:id "first-attempt-problematic"]{First attempt (problematic!)}

◊alectryon{Module FirstAttempt.}

◊p{The ◊code{RiverCrossing} predicate represents the state of the problem. It
...
◊p{We have the initial state, where all objects are on the left bank, the
right bank is empty, and the farmer is going right. I used the ◊hyperlink["https://coq.inria.fr/library/Coq.Lists.List.html#In"]{◊code{In}
predicate} to
avoid having to deal with specific list orderings.}

◊p{Then the farmer can go right or left, alone. In that case, the
contents of both banks do not change, but we must enforce that the
objects that were just left alone do not eat each other.}

◊p{Else, the farmer takes one object along the trip. I just checked that
...
◊p{For reference, I’ll just reproduce the proof using this model below,
...
◊code{RiverCrossing} out of thin air.}
...
◊p{I was quite satisfied that this worked out, but luckily that model was
amenable to simple proof automation. Using the ◊hyperlink["https://coq.inria.fr/refman/proofs/automatic-tactics/auto.html#coq:tacn.info_eauto"]{◊code{info_eauto}
tactic},
it proved that the objects could go all to the right side... But by a
strange path...}

◊p{I also needed to add the constructors and functions relevant to the
problem in a hint database called ◊code{local} so that ◊code{eauto} could use
them.}

◊alectryon{Hint Constructors Object : local.
...
◊h1[#:id "second-attempt"]{Second attempt}

◊alectryon{Module SecondAttempt.}

◊p{In my second attempt, I only changed the ◊code{goRight} and ◊code{goLeft}
...
◊alectryon{Inductive RiverCrossing :
  Direction -> list Object -> list Object -> Prop :=
| initial : forall l,
    In Wolf l ->
    In Goat l ->
...
◊p{Unfortunately, with those changes, neither ◊code{auto} nor ◊code{eauto} work
anymore. There are some goals that are repeated during the proof,
especially regarding goals and hypothesis of the forms ◊code{In X Xs} and
◊code{~ (In X Xs)}. Using some excerpts from the
◊hyperlink["http://adam.chlipala.net/cpdt/"]{CPDT} and
◊hyperlink["https://softwarefoundations.cis.upenn.edu/plf-current/index.html"]{PLF}
books, I managed to make some rudimentary automated tactics for those
cases.}

◊alectryon{Ltac auto_not_in :=
...
◊alectryon{End SecondAttempt.}

◊h1[#:id "conclusions-and-issues"]{Conclusions and Issues}

◊p{In the end, I was surprised that I was able to produce something that
...
◊p{But there are still some issues that I cannot address on my own, given
my current knowledge.}

◊ol{
◊li{I’m not sure if there is another "hole" in my logic, as the first
time everything fit into place but was wrong.}
◊li{The ◊code{RiverCrossing} predicate and the solution still seem overly
complicated. I feel that they should be shorter and cleaner...}
◊li{Both proofs are backwards. I’d like to somehow start from the
initial state and build my proof towards the desired goal. I guess
one way is to "reverse" the constructors in ◊code{RiverCrossing} and
start from the initial state in the solution goal. But that seems
wrong.}
◊li{I still need to specify the initial left and right river banks on
the steps that carry an object. I expected that I could use
◊code{eapply} or ◊code{eauto} and let Coq infer those later, yet that didn’t
work out. This is kinda annoying.}
◊li{If I were to use
◊hyperlink["https://coq.inria.fr/refman/proofs/writing-proofs/proof-mode.html#bullets"]{bullets}
...
I ◊hyperlink["https://coq.zulipchat.com/#narrow/stream/237977-Coq-users/topic/.5Bnewbie.5D.20wolf.2C.20goat.20and.20cabbage.20puzzle/near/252471119"]{shared this problem in the Coq
Zulip}
