...
how Elixir/Erlang break up stack frames).

#+BEGIN_SRC
  Each rectangle is a continuation frame, ● are the places
  where the redexes go into.

  ┌──────────┐
  │  2 * ●  │   redex: shift cont do
  │          │            3 * cont(4)
  │          │          end
  ├──────────┤
  │ reset ● │
...
Which reduces to ~25~, as said before.
